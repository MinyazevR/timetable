...
async def process_all_fields():
    ...
    for faculty in col_of_faculty.findAll('li',
                                          attrs={'class': 'list-group-item'}):
        faculty_tag = faculty.find('a')
        faculty_link = faculty_tag.get('href')
        # name = direction.text

        faculty_html = bs4.BeautifulSoup(markup=(await get_html(host +
                                                                faculty_link)),
                                         features="lxml")
        for panel_of_fields in faculty_html.findAll(
            ...
            for field in panel_of_fields.findAll(
                ...
                for tag_a in tags_a:
                    ...
                    try:
                        field_id = int(fields_link[fields_link.rfind('/') +
                                                   1:])
