...
async def delete_events(left_date: datetime, right_date: datetime) -> None:
    async with engine.connect() as session:
        await session.execute(text('DELETE From "Event" '
                                   'WHERE "Event".start_time >= (:left_date) AND "Event".end_time <= (:right_date)'),
                              {'left_date': left_date, 'right_date': right_date})
        await session.commit()


async def fill_event_table(dt_start: datetime, dt_end: datetime, subject: str, location: str) -> int:
    """Function for filling in the Event table"""
    async with engine.connect() as session:
        for i in range(2):
            result = await execute_insert(session, text(
                'WITH "ids" AS ('
                ...
                'and description=(:description) and location=(:location)));'),
                                          {'start_time': dt_start, 'end_time': dt_end,
                                           'description': subject, 'location': location})
...
async def fill_group_to_event_table(group_id: int, event_id: int):
    ...
    async with engine.connect() as session:
        await execute_insert(session, text(
            ...
            'DO NOTHING'), {'group_id': group_id, 'event_id': event_id})
...
def time_handler(times: str, current_year: int, header_date: datetime) -> tuple[str, str]:
    ...
    if len(times) >= 5:
        fixed_dt_time = datetime.datetime(year=current_year, day=header_date.day, month=header_date.month)
        start_delta = datetime.timedelta(hours=int(times[0:2]), minutes=int(times[3:5]))
        start_time = fixed_dt_time + start_delta
        if len(times) >= 11:
            end_delta = datetime.timedelta(hours=0, minutes=0, seconds=0)
            try:
                end_delta = datetime.timedelta(hours=int(times[6:8]), minutes=int(times[9:]))
            except ValueError:
                # logger
                pass
...
async def event_crawler(html: str, user_id: int, initial_date: datetime, groups: dict[str, str]) -> None:
    ...
    for day in day_panel:
        ...
        header_date = datetime.datetime(day=int(day_dt), month=dct[month_name], year=initial_year)
        ...
        panel_for_the_day = day.findNext("ul", attrs={'class': 'panel-collapse nopadding nomargin'})
        ...
        for event in panel_for_the_day.findAll('li', attrs={'class': 'common-list-item row'}):

            # Обработка времени
            time_element = event.find('div', attrs={'class': "col-sm-2 studyevent-datetime"})

            # Смотрим отменено событие или нет
            cancelled = time_element.find('span', attrs={'class': "moreinfo cancelled"})
            ...
            start_time, end_time = time_handler(times, current_year, header_date)

            # Обработка Дисциплины
            subject_element = event.find('div', attrs={'class': "col-sm-4 studyevent-subject"})

            # Пока что обрезал до 256, но может быть длинее
            subject_element = subject_element.find('span', attrs={'class': 'moreinfo'})
            ...
            location_and_groups_element = event.findAll('div', attrs={'class': 'col-sm-3'})

            location = "" if len(location_and_groups_element) == 0 else process_timetable_element(
            ...
            groups_name = "" if len(location_and_groups_element) < 2 else process_timetable_element(
                location_and_groups_element[1])

            # Ксли время не указано, то в качестве значения по умолчанию берется начало unix-time
            dt_start = datetime.datetime(year=1970, day=1, month=1) if start_time == "" else datetime.datetime.strptime(
                start_time, '%Y%m%dT%H%M%SZ')
            dt_end = datetime.datetime(year=1970, day=1, month=1) if end_time == "" else datetime.datetime.strptime(
                end_time, '%Y%m%dT%H%M%SZ')

            # Заполняем таблицу Event
            event_id = await fill_event_table(dt_start, dt_end, subject, location)
...
async def generate_week_url(user_id: int, left_date: datetime, right_date: datetime, groups: dict[str, str]):
    ...
    while copy_left_date < right_date:
        ...
        task_list.append(
            asyncio.create_task(get_html(f"https://timetable.spbu.ru/WeekEducatorEvents/{user_id}/{url_part}")))
...
async def fill_event_table_with_interval(left_border: datetime, right_border: datetime) -> None:
    ...
    async with engine.connect() as lol:
        groups = {x: y for x, y in (await lol.execute(text('SELECT name, id FROM "Group"'))).fetchall()}
        user_row = (await lol.execute(text('SELECT id FROM "User"'))).fetchall()

    lst = []
    for elem in user_row:
        lst.append(asyncio.create_task(generate_week_url(elem[0], left_border, right_border, groups)))
